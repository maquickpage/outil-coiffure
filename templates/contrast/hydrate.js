...
function buildTestimonials(testimonials) {
  ...
  if (!row) return;
  const items = ((testimonials && testimonials.items) || []).slice(0, 3);
  // Robustesse : aucun avis → on masque la bande entière.
  ...
  // Citations aérées en colonnes (pas de carrousel) — style bande éditoriale.
...
// Galerie : grille classique (grid ou masonry selon gallery.layout). 6 photos
// visibles, puis « Afficher plus » (+6). Clic/Entrée → lightbox. Robuste :
// liste vide → section masquée.
function buildGallery(gallery) {
  ...
  galleryAllImages = ((gallery && gallery.images) || []).slice();
  // Robustesse : pas d'image → on masque la section galerie.
  if (galleryAllImages.length === 0) { if (section) section.style.display = 'none'; return; }
  if (section) section.style.display = '';

  const masonry = !!(gallery && gallery.layout === 'masonry');
  galleryShownCount = Math.min(6, galleryAllImages.length);
  grid.classList.toggle('layout-masonry', masonry);
  // Bento (rythme de tuiles variées) : uniquement en layout grid ET si assez de
  // photos, sinon on retombe sur une grille régulière (pas de tuile géante isolée).
  grid.classList.toggle('bento', !masonry && galleryAllImages.length >= 3);
...
function renderGalleryItems() {
  ...
  grid.innerHTML = visible.map((img, i) => `
    <div class="gallery-item" data-index="${i}" role="button" tabindex="0" aria-label="Agrandir la réalisation ${i + 1}">
      <img src="${escapeHtml(img)}" alt="Réalisation ${i + 1}" loading="lazy">
      <div class="gallery-overlay"><span>Agrandir</span></div>
    </div>
  `).join('');
  grid.querySelectorAll('.gallery-item').forEach(item => {
    const open = () => openLightbox(galleryAllImages, parseInt(item.dataset.index));
...
// centrées verticalement sur toute la hauteur du slide, donc perdues au milieu
// d'une longue liste ; ici elles encadrent le titre, en haut, toujours visibles.
// Le libellé garde la classe .services-cat-label pour hériter de la typo propre
// à chaque template.
function ensureCarouselHead(wrapper, prev, next) {
  if (!wrapper) return null;
  let head = wrapper.querySelector('.services-carousel-head');
  if (head) return head;
...
function renderServices() {
  ...
  if (!useCarousel) {
    ...
    const existingHead = wrapper && wrapper.querySelector('.services-carousel-head');
    if (existingHead) existingHead.hidden = true;
  ...
  // Le nom de catégorie sort du slide pour rejoindre l'en-tête fixe, entre les
  // deux flèches. Il ne défile donc plus avec le contenu.
  ...
  const head = ensureCarouselHead(wrapper, prev, next);
  ...
  const update = () => {
    ...
    syncHeight();
    const label = head && head.querySelector('.services-carousel-cat');
    // Pas de libellé quand les pages ne sont pas des catégories (cas « une seule
    // catégorie, liste longue » découpée en paquets) → on affiche le rang.
...
function applyHeroImage(src) {
  ...
  // La photo est portee par .hero::before (pour le filtre cinema) via cette
  // variable CSS. Voir styles.css §HERO.
  if (hero) hero.style.setProperty('--hero-img', `url('${src}')`);
}
